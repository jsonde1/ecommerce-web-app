...
export default class ListingServices {
  ...
  async getListings(provisional = false, userID = 0) {
    ...
    // Not yet implemented
    let sqlQuery = `SELECT Listings.id, Listings.Title, Listings.Condition, Listings.Description, Listings.Price, Listings.MainImage, Listings.CreationDate, Listings.UserID, Listings.Status, Users.Name, Users.PhoneNumber FROM Listings LEFT JOIN Users ON Users.id = Listings.UserID WHERE NOT Listings.UserID = @UserID;`;
